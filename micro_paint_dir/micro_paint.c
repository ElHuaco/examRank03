...
/*   Updated: 2020/11/06 12:18:10 by aleon-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	belongs_to_rectangle(float x, float y, t_rectangle *rect)
{
	...
	{
		if (((x - rect->xtl) < 1) || ((xbr - x) < 1)
			|| ((y - rect->ytl) < 1) || ((ybr - y) < 1))
			return (1);
		else
			return (0);
	}
...
static int	output_update(char ***output, t_rectangle *rect)
{
	...
	if (((rect->type != 'R') && (rect->type != 'r')) || (rect->width <= 0)
		|| (rect->height <= 0))
	{
		free_array(*output);
		return (1);
...
static int	execute_operations_and_draw(FILE *op_file)
{
	...
	if ((set_dim_and_bg(op_file, &output)))
		return (1);
	while ((scan_ret = fscanf(op_file, "%c %f %f %f %f %c\n", &rect.type,
